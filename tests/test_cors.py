...
def test_cors_headers_on_main_endpoint():
    ...
    headers = {
        "Origin": "https://kzml3cjwnshszfymqji0.lite.vusercontent.net",
        "Access-Control-Request-Method": "GET",
    }
    response = client.get("/", headers=headers)
    assert response.status_code == 200
    assert response.headers["access-control-allow-origin"] == "https://kzml3cjwnshszfymqji0.lite.vusercontent.net"
...
def test_cors_disallowed_origin():
    """Test CORS headers are not present for disallowed origins"""
    headers = {
...
def test_cors_preflight_post():
    """Test OPTIONS request handling for POST method"""
    headers = {
        "Origin": "https://kzml3cjwnshszfymqji0.lite.vusercontent.net",
...
def test_cors_header_exposure():
    ...
    response = client.get("/", headers=headers)
    assert response.status_code == 200
    assert "*" in response.headers["access-control-expose-headers"]
